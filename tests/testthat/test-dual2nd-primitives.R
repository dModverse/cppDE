...
run_modes <- function(expr, parameters, x_vals, tangentP, hessianP = NULL) {
  f <- d2prim[[name_of(expr)]]
  stopifnot(!is.null(f), identical(attr(f, "parameters"), parameters))
  args <- as.list(x_vals)
  args$tangentP <- tangentP
  if (!is.null(hessianP)) args$hessianP <- hessianP
  args$deriv2 <- TRUE
  ...
  list(forward = do.call(f$evaluate, args),
       reference = list(y = matrix(at(e), 1, 1), tangent = dy, hessian = d2y))
}
...
expect_modes_agree <- function(out, tol_y = 1e-12, tol_dy = 1e-10, tol_d2y = 1e-10) {
  ...
  expect_equal(unname(out$forward$tangent), unname(out$reference$tangent),
               tolerance = tol_dy)
  expect_equal(unname(out$forward$hessian), unname(out$reference$hessian),
               tolerance = tol_d2y)
}

# Identity Phi(theta) = theta: tangentP = I, hessianP = 0. Each parameter is
# its own theta direction so the raw Jacobian / Hessian come through.
...
test_that("dual2nd matches D() on a + b", {
  ...
  out <- run_modes(c(y = "a + b"), parameters = c("a", "b"),
                   x_vals = list(a = 1.5, b = 2.5), tangentP = s$tangentP,
                   hessianP = s$hessianP)
...
test_that("dual2nd matches D() on sin / cos / tan", {
  ...
  for (fn in c("sin", "cos", "tan")) {
    ...
    out  <- run_modes(expr, parameters = "x",
                      x_vals = list(x = 0.7), tangentP = s$tangentP,
                      hessianP = s$hessianP)
    expect_modes_agree(out)
  }
})
...
test_that("dual2nd matches D() on exp / log / sqrt", {
  ...
  for (fn in c("exp", "log", "sqrt")) {
    expr <- setNames(sprintf("%s(x)", fn), "y")
    out  <- run_modes(expr, parameters = "x",
                      x_vals = list(x = 1.7), tangentP = s$tangentP,
                      hessianP = s$hessianP)
    expect_modes_agree(out)
  }
})
...
test_that("dual2nd matches D() on hyperbolic trig", {
  ...
  for (fn in c("sinh", "cosh", "tanh")) {
    expr <- setNames(sprintf("%s(x)", fn), "y")
    out  <- run_modes(expr, parameters = "x",
                      x_vals = list(x = 0.5), tangentP = s$tangentP,
                      hessianP = s$hessianP)
    expect_modes_agree(out)
  }
})
...
test_that("dual2nd matches D() on a^b (both AD)", {
  s <- identity_seeds(c("a", "b"))
  out <- run_modes(c(y = "a^b"), parameters = c("a", "b"),
                   x_vals = list(a = 1.7, b = 2.3), tangentP = s$tangentP,
                   hessianP = s$hessianP)
  expect_modes_agree(out)
})
...
test_that("dual2nd matches D() on a^2 (scalar exponent)", {
  ...
  out <- run_modes(c(y = "a^2"), parameters = c("a"),
                   x_vals = list(a = 1.7), tangentP = s$tangentP,
                   hessianP = s$hessianP)
  expect_modes_agree(out)
})
...
test_that("dual2nd matches D() on a*sin(b) + exp(a)", {
  ...
  out <- run_modes(c(y = "a*sin(b) + exp(a)"), parameters = c("a", "b"),
                   x_vals = list(a = 0.4, b = 1.1), tangentP = s$tangentP,
                   hessianP = s$hessianP)
  expect_modes_agree(out)
})
...
test_that("dual2nd output is Hessian-symmetric (mirror via dd_raw)", {
  ...
  out <- run_modes(c(y = "a*b + b*c + a*c"), parameters = c("a", "b", "c"),
                   x_vals = list(a = 1, b = 2, c = 3), tangentP = s$tangentP,
                   hessianP = s$hessianP)
  d2y <- out$forward$hessian
