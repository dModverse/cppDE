...
# -----------------------------------------------------------------
.workingDir <- file.path(tempdir(), "cppDE_cascade_benchmark")
...
cppDE:::compile(m_ndf, m_bdf, m_tsit5, m_rb4, m_cvode, m_cvode_klu, cores = 6)
cat("Done.\n\n")
...
mb <- microbenchmark(
  `cppDE BDF`       = solveODE(m_bdf, times, params, abstol = .abstol, reltol = .reltol),
  `cppDE NDF`       = solveODE(m_ndf, times, params, abstol = .abstol, reltol = .reltol),
  `cppDE RB4`       = solveODE(m_rb4, times, params, abstol = .abstol, reltol = .reltol),
  `cppDE TSIT5`     = solveODE(m_tsit5, times, params, abstol = .abstol, reltol = .reltol),
...
norms <- list(
  `cppDE BDF        vs deSolve lsodes()` = norm(align(toWide(res_bdf))       - wide_ref, type = "I"),
  `cppDE NDF        vs deSolve lsodes()` = norm(align(toWide(res_ndf))       - wide_ref, type = "I"),
  `cppDE RB4        vs deSolve lsodes()` = norm(align(toWide(res_rb4))       - wide_ref, type = "I"),
  `cppDE TSIT5      vs deSolve lsodes()` = norm(align(toWide(res_tsit5))     - wide_ref, type = "I"),
