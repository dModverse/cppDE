...
.workingDir <- file.path(tempdir(), "cppDE_bench_stiff")
...
print_row("cppDE bdf", ncol(r$variable), t_bdf, r$diagnostics, rob_mass_err_cpp(r))

r <- solveODE(rob_rb4, rob_times, rob_pars)
t_rb4 <- bench("rb4", quote(solveODE(rob_rb4, rob_times, rob_pars)), nrep)
print_row("cppDE rb4", ncol(r$variable), t_rb4, r$diagnostics, rob_mass_err_cpp(r))
...
cat(sprintf("\n  Max |cppDE rb4 - deSolve lsoda|: %.3e\n",
...
print_row("cppDE bdf", ncol(r$variable), t_bdf, r$diagnostics)

r <- solveODE(vdp_rb4, vdp_times, vdp_pars)
t_rb4 <- bench("rb4", quote(solveODE(vdp_rb4, vdp_times, vdp_pars)), nrep)
print_row("cppDE rb4", ncol(r$variable), t_rb4, r$diagnostics)
...
print_row("cppDE bdf", ncol(r_bdf$variable), t_bdf, r_bdf$diagnostics, rob_mass_err_cpp(r_bdf))
...
print_row("cppDE ndf", ncol(r_ndf$variable), t_ndf, r_ndf$diagnostics, rob_mass_err_cpp(r_ndf))
...
cvode_bin <- file.path(dirname(normalizePath(
  if (nzchar(Sys.getenv("CPPDE_BENCH_SCRIPT")))
    Sys.getenv("CPPDE_BENCH_SCRIPT") else tempdir())),
  "cvode_robertson")
