...
.workingDir <- file.path(tempdir(), "cppDE_bench_explicit")
...
print_row("cppDE tsit5", ncol(r$variable), t1, r$diagnostics, err)

r <- solveODE(ho_adams, ho_times, ho_pars)
err <- max(abs(r$variable["x",] - ho_ref_x), abs(r$variable["y",] - ho_ref_y))
t1 <- bench("adams", quote(solveODE(ho_adams, ho_times, ho_pars)), nrep)
print_row("cppDE adams", ncol(r$variable), t1, r$diagnostics, err)
...
print_row("cppDE tsit5", ncol(r$variable), t1, r$diagnostics)

r <- solveODE(pl_adams, pl_times, pl_pars)
t1 <- bench("adams", quote(solveODE(pl_adams, pl_times, pl_pars)), nrep)
print_row("cppDE adams", ncol(r$variable), t1, r$diagnostics)
