...
run_problem <- function(prob) {
  ...
  # Compile cppDE models (bdf/ndf, and optionally rb4)
  cppde_models <- list()
  cat("  compiling cppDE models...\n")
  cppde_models$ndf <- cppODE(prob$rhs, deriv = FALSE, outdir = getwd(),
                              method = "bdf", sparse = prob$sparse,
                              modelname = paste0(prob$id, "_ndf"), compile = TRUE)
  cppde_models$bdf <- cppODE(prob$rhs, deriv = FALSE, outdir = getwd(),
                              method = "bdf", sparse = prob$sparse, useNDF = FALSE,
                              modelname = paste0(prob$id, "_bdf"), compile = TRUE)
  if (isTRUE(prob$include_rb4)) {
    cppde_models$rb4 <- cppODE(prob$rhs, deriv = FALSE, outdir = getwd(),
  ...
  for (tol_name in names(TOLS)) {
    ...
    for (sname in names(cppde_models)) {
      local({
        m <- cppde_models[[sname]]
        label <- paste0("cppDE_", sname)
...
problems$pollution <- local({
  ...
  # For cppDE we need symbolic rhs. Use the named vector approach.
  rhs <- c(
