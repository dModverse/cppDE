...
.workingDir <- file.path(tempdir(), "cppDE_example_coupled_vanderpol")
...
cat(sprintf("  cppDE sparse (no sens): %.1f s\n", t_compile_nosens_sparse["elapsed"]))


cat("  Compiling cppDE dense model (deriv = FALSE)...\n")
...
cat(sprintf("  cppDE dense (no sens): %.1f s\n", t_compile_nosens_dense["elapsed"]))

# cppDE: WITH sensitivities (AD)
cat("  Compiling cppDE sparse BDF (deriv = TRUE)...\n")
t_compile_cppde_sparse <- system.time({
  funcpp_sparse <- cppODE(rhs, outdir = getwd(), method = "bdf",
                          modelname = sprintf("vdp%d_r%d_sparse_sens", N, reach),
                          compile = TRUE, deriv = TRUE)
})
cat(sprintf("  cppDE sparse BDF (AD): %.1f s\n", t_compile_cppde_sparse["elapsed"]))

cat("  Compiling cppDE sparse RB4 (deriv = TRUE)...\n")
t_compile_cppde_sparse_rb <- system.time({
  funcpp_sparse_rb <- cppODE(rhs, outdir = getwd(), method = "rb4",
                             modelname = sprintf("vdp%d_r%d_sparse_sens_rb", N, reach),
                             compile = TRUE, deriv = TRUE)
})
cat(sprintf("  cppDE sparse RB4 (AD): %.1f s\n", t_compile_cppde_sparse_rb["elapsed"]))

cat("  Compiling cppDE dense BDF (deriv = TRUE)...\n")
t_compile_cppde_dense <- system.time({
...
cat(sprintf("  cppDE dense BDF (AD): %.1f s\n", t_compile_cppde_dense["elapsed"]))

cat("  Compiling cppDE dense RB4 (deriv = TRUE)...\n")
t_compile_cppde_dense_rb <- system.time({
  funcpp_dense_rb <- cppODE(rhs, outdir = getwd(), method = "rb4",
                            modelname = sprintf("vdp%d_r%d_dense_sens_rb", N, reach),
                            compile = TRUE, deriv = TRUE, sparse = FALSE)
})
cat(sprintf("  cppDE dense RB4 (AD): %.1f s\n", t_compile_cppde_dense_rb["elapsed"]))
...
cat("\n--- cppDE BDF sparse (AD sensitivities) ---\n")
cat(sprintf("  ODE dimension: %d (dual numbers propagate sensitivities)\n", n_states))
t_cppde_sparse <- system.time({
  res_cppde_sparse <- solveODE(funcpp_sparse, times, params)
})
cat(sprintf("  Time: %.3f s\n", t_cppde_sparse["elapsed"]))
cppDE::diagnostics(res_cppde_sparse)

cat("\n--- cppDE RB4 sparse (AD sensitivities) ---\n")
t_cppde_sparse_rb <- system.time({
  res_cppde_sparse_rb <- solveODE(funcpp_sparse_rb, times, params)
})
cat(sprintf("  Time: %.3f s\n", t_cppde_sparse_rb["elapsed"]))
cppDE::diagnostics(res_cppde_sparse_rb)

cat("\n--- cppDE BDF dense (AD sensitivities) ---\n")
t_cppde_dense <- system.time({
  res_cppde_dense <- solveODE(funcpp_dense, times, params)
})
cat(sprintf("  Time: %.3f s\n", t_cppde_dense["elapsed"]))
cppDE::diagnostics(res_cppde_dense)

cat("\n--- cppDE RB4 dense (AD sensitivities) ---\n")
t_cppde_dense_rb <- system.time({
  res_cppde_dense_rb <- solveODE(funcpp_dense_rb, times, params)
})
cat(sprintf("  Time: %.3f s\n", t_cppde_dense_rb["elapsed"]))
cppDE::diagnostics(res_cppde_dense_rb)
...
# cppDE no-sens baselines
for (info in list(
  list(res = res_nosens_sparse, lab = "cppDE sparse (no sens)"),
  list(res = res_nosens_dense,  lab = "cppDE dense (no sens)")
)) {
  res <- info$res
...
# cppDE with AD sensitivities
for (info in list(
  list(res = res_cppde_sparse,    lab = "cppDE BDF sparse (AD)"),
  list(res = res_cppde_sparse_rb, lab = "cppDE RB4 sparse (AD)"),
  list(res = res_cppde_dense,     lab = "cppDE BDF dense (AD)"),
  list(res = res_cppde_dense_rb,  lab = "cppDE RB4 dense (AD)")
)) {
  res <- info$res
  if (!is.null(res) && length(res$time) > 1) {
    df_plots <- c(df_plots, list(data.frame(
...
print_cppde_row <- function(label, elapsed, res) {
...
print_cppde_row("cppDE BDF sparse (no sens)",
                 t_nosens_sparse["elapsed"], res_nosens_sparse)
print_cppde_row("cppDE BDF dense (no sens)",
...
print_cppde_row("cppDE BDF sparse (AD)",
                 t_cppde_sparse["elapsed"], res_cppde_sparse)
print_cppde_row("cppDE RB4 sparse (AD)",
                 t_cppde_sparse_rb["elapsed"], res_cppde_sparse_rb)
print_cppde_row("cppDE BDF dense (AD)",
                 t_cppde_dense["elapsed"], res_cppde_dense)
print_cppde_row("cppDE RB4 dense (AD)",
                 t_cppde_dense_rb["elapsed"], res_cppde_dense_rb)
