...
.workingDir <- file.path(tempdir(), "cppDE_example_forcings")
...
# -- Helper: extract states, tangent, hessian into long data.table --
extract_long <- function(res, system_label) {

  # Layouts: variable [n_out, n_vars]; tangent [n_out, n_vars, n_sens];
  # hessian [n_out, n_vars, n_sens, n_sens]
  states <- dimnames(res$tangent)[[2]]  # e.g. c("x", "v")
  pars   <- dimnames(res$tangent)[[3]]  # e.g. c("x", "v", "omega0", "gamma")
  nt     <- length(res$time)
  ...
  # -- First-order sensitivities: tangent[time, state, par] --
  sens <- res$tangent
  dt_tangent <- rbindlist(lapply(seq_len(nt), function(i) {
  ...
  dt_tangent[, type := "tangent"]

  # -- Second-order sensitivities: hessian[time, state, par, par] --
  hessian <- res$hessian
  np <- length(pars)
  # Lower-triangular indices (including diagonal)
  idx <- which(lower.tri(matrix(1, np, np), diag = TRUE), arr.ind = TRUE)

  dt_hessian <- rbindlist(lapply(seq_len(nt), function(i) {
    rbindlist(lapply(states, function(s) {
      mat_i <- hessian[i, s, , ]  # [par x par]
      data.table(
        time  = res$time[i],
  ...
  dt_hessian[, type := "hessian"]

  # -- Combine --
  dt <- rbindlist(list(dt_states, dt_tangent, dt_hessian))
...
for (tp in c("state", "tangent", "hessian")) {
