/*
 Expression Templates for cppde::dual2nd<T, N>.

 Mirrors the structure of cppde_dual_expr.hpp (first-order ETs) lifted to
 ...
 dual2nd::operator=(const Expr2nd<D>&) (declared in cppde_dual2nd.hpp,
 defined here below).
...
#ifndef CPPDE_DUAL2ND_EXPR_HPP
#define CPPDE_DUAL2ND_EXPR_HPP
...
#else
  #define CPPDE_ET2_INLINE inline
#endif
...
struct Expr2nd {
  CPPDE_ET2_INLINE const Derived& self() const { return *static_cast<const Derived*>(this); }
};
...
struct Dual2ndLeaf : Expr2nd<Dual2ndLeaf<T, N>> {
  ...
  CPPDE_ET2_INLINE explicit Dual2ndLeaf(const dual2nd<T, N>& d)
    : src_(d), x_(d.scalar()), depend_(d.depend()),
      m_(d.depend() ? d.width() : 0u) {}

  CPPDE_ET2_INLINE T eval_value() const { return x_; }
  CPPDE_ET2_INLINE T eval_d1(unsigned i) const {
    return depend_ ? src_.d1_at(i) : T(0);
  }
  CPPDE_ET2_INLINE T eval_d2(unsigned i, unsigned j) const {
    return depend_ ? src_.dd_at(i, j) : T(0);
  }
  CPPDE_ET2_INLINE bool any_depend() const { return depend_; }
...
struct ScalarLeaf2 : Expr2nd<ScalarLeaf2<T>> {
  ...
  T v_;
  CPPDE_ET2_INLINE explicit ScalarLeaf2(T v) : v_(v) {}
  CPPDE_ET2_INLINE T eval_value() const { return v_; }
  CPPDE_ET2_INLINE T eval_d1(unsigned)             const { return T(0); }
  CPPDE_ET2_INLINE T eval_d2(unsigned, unsigned)   const { return T(0); }
  CPPDE_ET2_INLINE bool any_depend() const { return false; }
  CPPDE_ET2_INLINE unsigned width() const { return 0u; }
};
...
struct BinExpr2 : Expr2nd<BinExpr2<L, R, Op>> {
  ...
  CPPDE_ET2_INLINE BinExpr2(L l, R r)
  ...
  CPPDE_ET2_INLINE value_type eval_value() const { return y_; }
  ...
  CPPDE_ET2_INLINE bool any_depend() const { return dep_; }
  CPPDE_ET2_INLINE unsigned width() const { return m_; }
};
...
struct BinScalarRight2 : Expr2nd<BinScalarRight2<L, Op, T>> {
  ...
  CPPDE_ET2_INLINE BinScalarRight2(L l, T s)
  ...
  CPPDE_ET2_INLINE T eval_value() const { return y_; }
  CPPDE_ET2_INLINE T eval_d1(unsigned i) const {
  ...
  CPPDE_ET2_INLINE bool any_depend() const { return l_.any_depend(); }
  CPPDE_ET2_INLINE unsigned width() const { return l_.width(); }
};
...
struct BinScalarLeft2 : Expr2nd<BinScalarLeft2<R, Op, T>> {
  ...
  CPPDE_ET2_INLINE BinScalarLeft2(T s, R r)
  ...
  CPPDE_ET2_INLINE bool any_depend() const { return r_.any_depend(); }
  CPPDE_ET2_INLINE unsigned width() const { return r_.width(); }
};
...
#define CPPDE_DEFINE_ET2_UNARY(NAME, VAL_EXPR, FP_EXPR, FPP_EXPR)            \
  struct NAME ## Op2 {                                                        \
    template<class T> static void compute(const T& xv, T& y, T& fp, T& fpp) {  \
...
CPPDE_DEFINE_ET2_UNARY(Exp,  exp(xv),  y,  y);
CPPDE_DEFINE_ET2_UNARY(Log,  log(xv),  T(1) / xv,  -T(1) / (xv * xv));
CPPDE_DEFINE_ET2_UNARY(Sqrt, sqrt(xv), T(1) / (T(2) * y),
                              -T(1) / (T(4) * y * y * y));
CPPDE_DEFINE_ET2_UNARY(Sin,  sin(xv),  cos(xv), -y);
CPPDE_DEFINE_ET2_UNARY(Cos,  cos(xv), -sin(xv), -y);
CPPDE_DEFINE_ET2_UNARY(Tan,  tan(xv),  T(1) + y * y,
                              T(2) * y * (T(1) + y * y));
CPPDE_DEFINE_ET2_UNARY(Asin, asin(xv),
                              T(1) / sqrt(T(1) - xv * xv),
                              xv / ((T(1) - xv * xv) * sqrt(T(1) - xv * xv)));
CPPDE_DEFINE_ET2_UNARY(Acos, acos(xv),
                             -T(1) / sqrt(T(1) - xv * xv),
                             -xv / ((T(1) - xv * xv) * sqrt(T(1) - xv * xv)));
CPPDE_DEFINE_ET2_UNARY(Atan, atan(xv),
                              T(1) / (T(1) + xv * xv),
                             -T(2) * xv / ((T(1) + xv * xv) * (T(1) + xv * xv)));
CPPDE_DEFINE_ET2_UNARY(Sinh, sinh(xv), cosh(xv), y);
CPPDE_DEFINE_ET2_UNARY(Cosh, cosh(xv), sinh(xv), y);
CPPDE_DEFINE_ET2_UNARY(Tanh, tanh(xv), T(1) - y * y,
                             -T(2) * y * (T(1) - y * y));
CPPDE_DEFINE_ET2_UNARY(Asinh, asinh(xv),
                               T(1) / sqrt(xv * xv + T(1)),
                              -xv / ((xv * xv + T(1)) * sqrt(xv * xv + T(1))));
CPPDE_DEFINE_ET2_UNARY(Acosh, acosh(xv),
                               T(1) / sqrt(xv * xv - T(1)),
                              -xv / ((xv * xv - T(1)) * sqrt(xv * xv - T(1))));
...
struct UnaryExpr2 : Expr2nd<UnaryExpr2<A, Op>> {
  ...
  CPPDE_ET2_INLINE explicit UnaryExpr2(A a) : a_(std::move(a)) {
  ...
  CPPDE_ET2_INLINE bool any_depend() const { return a_.any_depend(); }
  CPPDE_ET2_INLINE unsigned width() const { return a_.width(); }
};
...
struct PowExpr2DD : Expr2nd<PowExpr2DD<L, R>> {
  ...
  CPPDE_ET2_INLINE PowExpr2DD(L l, R r)
    : l_(std::move(l)), r_(std::move(r))
  {
...
struct PowExpr2DS : Expr2nd<PowExpr2DS<L>> {
  ...
  CPPDE_ET2_INLINE PowExpr2DS(L l, value_type b)
    : l_(std::move(l)), bv_(b)
  {
    using std::pow;
    using T = value_type;
  ...
  CPPDE_ET2_INLINE value_type eval_value() const { return y_; }
  CPPDE_ET2_INLINE value_type eval_d1(unsigned i) const { return pa_ * l_.eval_d1(i); }
  CPPDE_ET2_INLINE value_type eval_d2(unsigned i, unsigned j) const {
...
struct PowExpr2SD : Expr2nd<PowExpr2SD<R>> {
  ...
  CPPDE_ET2_INLINE PowExpr2SD(value_type a, R r)
    : r_(std::move(r)), av_(a)
  {
    using std::pow; using std::log;
    using T = value_type;
  ...
  CPPDE_ET2_INLINE value_type eval_value() const { return y_; }
  CPPDE_ET2_INLINE value_type eval_d1(unsigned i) const { return pb_ * r_.eval_d1(i); }
  CPPDE_ET2_INLINE value_type eval_d2(unsigned i, unsigned j) const {
...
CPPDE_ET2_INLINE X make_leaf2(const X& x, TT* = nullptr) { return x; }

template<class TT, class T, unsigned N>
CPPDE_ET2_INLINE Dual2ndLeaf<T, N> make_leaf2(const dual2nd<T, N>& d, TT* = nullptr) {
  return Dual2ndLeaf<T, N>(d);
}

template<class TT, class S,
         std::enable_if_t<std::is_arithmetic<S>::value, int> = 0>
CPPDE_ET2_INLINE ScalarLeaf2<TT> make_leaf2(S s, TT* = nullptr) {
...
#define CPPDE_DEFINE_ET2_BINOP(SYM, OPTAG)                                   \
  /* dual2nd op dual2nd */                                                    \
  template<class T, unsigned N>                                               \
  CPPDE_ET2_INLINE auto operator SYM(const dual2nd<T, N>& a,                 \
                                      const dual2nd<T, N>& b) {               \
  ...
  CPPDE_ET2_INLINE auto operator SYM(const Expr2nd<D>& a,                    \
                                      const dual2nd<T, N>& b) {               \
  ...
  /* Expr2nd op Expr2nd */                                                    \
  template<class L, class R>                                                  \
  CPPDE_ET2_INLINE auto operator SYM(const Expr2nd<L>& a,                    \
                                      const Expr2nd<R>& b) {                  \
  ...
  CPPDE_ET2_INLINE auto operator SYM(const dual2nd<T, N>& a, U b) {          \
  ...
  template<class T, unsigned N, class U,                                      \
           std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>           \
  CPPDE_ET2_INLINE auto operator SYM(U a, const dual2nd<T, N>& b) {          \
  ...
  template<class D, class U,                                                  \
           std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>           \
  CPPDE_ET2_INLINE auto operator SYM(const Expr2nd<D>& a, U b) {             \
  ...
  template<class U, class D,                                                  \
           std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>           \
  CPPDE_ET2_INLINE auto operator SYM(U a, const Expr2nd<D>& b) {             \
    using T = typename D::value_type;                                         \
    return BinScalarLeft2<D, OPTAG, T>(static_cast<T>(a), b.self());          \
  }

CPPDE_DEFINE_ET2_BINOP(+, AddOp2)
CPPDE_DEFINE_ET2_BINOP(-, SubOp2)
CPPDE_DEFINE_ET2_BINOP(*, MulOp2)
CPPDE_DEFINE_ET2_BINOP(/, DivOp2)

#undef CPPDE_DEFINE_ET2_BINOP

// ===========================================================================
// Unary +/- on dual2nd / Expr2nd.
// ===========================================================================
template<class T, unsigned N>
CPPDE_ET2_INLINE auto operator+(const dual2nd<T, N>& a) {
  return Dual2ndLeaf<T, N>(a);
}
template<class D>
CPPDE_ET2_INLINE auto operator+(const Expr2nd<D>& a) {
  return a.self();
}
...
#define CPPDE_DEFINE_ET2_UNARY_FN(NAME, OPTAG)                               \
  template<class T, unsigned N>                                               \
  CPPDE_ET2_INLINE auto NAME(const dual2nd<T, N>& a) {                       \
    return UnaryExpr2<Dual2ndLeaf<T, N>, OPTAG>(Dual2ndLeaf<T, N>(a));        \
  }                                                                            \
  template<class D>                                                           \
  CPPDE_ET2_INLINE auto NAME(const Expr2nd<D>& a) {                          \
    return UnaryExpr2<D, OPTAG>(a.self());                                    \
  }

CPPDE_DEFINE_ET2_UNARY_FN(exp,   ExpOp2)
CPPDE_DEFINE_ET2_UNARY_FN(log,   LogOp2)
CPPDE_DEFINE_ET2_UNARY_FN(sqrt,  SqrtOp2)
CPPDE_DEFINE_ET2_UNARY_FN(sin,   SinOp2)
CPPDE_DEFINE_ET2_UNARY_FN(cos,   CosOp2)
CPPDE_DEFINE_ET2_UNARY_FN(tan,   TanOp2)
CPPDE_DEFINE_ET2_UNARY_FN(asin,  AsinOp2)
CPPDE_DEFINE_ET2_UNARY_FN(acos,  AcosOp2)
CPPDE_DEFINE_ET2_UNARY_FN(atan,  AtanOp2)
CPPDE_DEFINE_ET2_UNARY_FN(sinh,  SinhOp2)
CPPDE_DEFINE_ET2_UNARY_FN(cosh,  CoshOp2)
CPPDE_DEFINE_ET2_UNARY_FN(tanh,  TanhOp2)
...
// ===========================================================================
template<class T, unsigned N>
CPPDE_ET2_INLINE auto pow(const dual2nd<T, N>& a, const dual2nd<T, N>& b) {
  return PowExpr2DD<Dual2ndLeaf<T, N>, Dual2ndLeaf<T, N>>(
              Dual2ndLeaf<T, N>(a), Dual2ndLeaf<T, N>(b));
}
template<class T, unsigned N, class D>
CPPDE_ET2_INLINE auto pow(const dual2nd<T, N>& a, const Expr2nd<D>& b) {
  return PowExpr2DD<Dual2ndLeaf<T, N>, D>(Dual2ndLeaf<T, N>(a), b.self());
}
template<class D, class T, unsigned N>
CPPDE_ET2_INLINE auto pow(const Expr2nd<D>& a, const dual2nd<T, N>& b) {
  return PowExpr2DD<D, Dual2ndLeaf<T, N>>(a.self(), Dual2ndLeaf<T, N>(b));
}
template<class L, class R>
CPPDE_ET2_INLINE auto pow(const Expr2nd<L>& a, const Expr2nd<R>& b) {
  return PowExpr2DD<L, R>(a.self(), b.self());
}
template<class T, unsigned N, class U,
         std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>
CPPDE_ET2_INLINE auto pow(const dual2nd<T, N>& a, U b) {
  return PowExpr2DS<Dual2ndLeaf<T, N>>(Dual2ndLeaf<T, N>(a), static_cast<T>(b));
}
template<class D, class U,
         std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>
CPPDE_ET2_INLINE auto pow(const Expr2nd<D>& a, U b) {
  using T = typename D::value_type;
  return PowExpr2DS<D>(a.self(), static_cast<T>(b));
}
template<class U, class T, unsigned N,
         std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>
CPPDE_ET2_INLINE auto pow(U a, const dual2nd<T, N>& b) {
  return PowExpr2SD<Dual2ndLeaf<T, N>>(static_cast<T>(a), Dual2ndLeaf<T, N>(b));
}
template<class U, class D,
         std::enable_if_t<std::is_arithmetic<U>::value, int> = 0>
CPPDE_ET2_INLINE auto pow(U a, const Expr2nd<D>& b) {
  using T = typename D::value_type;
...
CPPDE_ET2_INLINE void materialise(dual2nd<T, N>& lhs, const E& e) {
  // 1. Scalar.
