...
namespace cppde {
namespace detail {
// Pin BLAS to one thread: threaded BLAS under concurrent solves
// oversubscribes and is not bit-reproducible across thread counts.
// Runtime APIs only -- the setenv() fallback was dropped: BLAS reads those
// vars at its own init (too late), and it clobbered OMP_NUM_THREADS via a
// setenv() that is not thread-safe.  Magic static replaces a racy flag.
inline void ensure_single_thread_blas() {
  static const bool done = [] {
    // 1. MKL runtime API (works even without MKL headers)
#ifdef _WIN32
