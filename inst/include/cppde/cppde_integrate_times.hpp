 ...
 * - Forward-mode AD support (cppde::dual / cppde::dual2nd)
...
// ============================================================================
// Scalar value extraction & AD type detection: from cppde_ad_traits.hpp
// ============================================================================

using cppde::ad_traits::scalar_value;
template<class T> using is_ad_type = cppde::ad_traits::is_ad<T>;
...
// models (cppde::dual / cppde::dual2nd) to compute the analytical
...
//      restart at order 1.  Detected via cppde::stepper_traits.
...
private:
 ...
 //
 // Re-estimate h0 from scratch via cppde_hin, using the remaining
 ...
 void init_stepper_after_event(State& x, Time t, Time& dt) {
   if constexpr (::cppde::needs_restart_after_event_v<Stepper>) {
     if constexpr (!std::is_same_v<DtEstimator, no_dt_estimator>) {
       ...
       const double h_est = odeint_utils::cppde_hin<value_type>(
...
{
 // Pin BLAS to one thread for this solve; see cppde_blas_threads.hpp for why
 // a threaded BLAS corrupts the AD kernels.  The guard sits here because every
 // stepper reaches one of the two integrate_times() overloads, while the dense
 // LU -- the only place the pin used to live -- is skipped entirely by sparse
 // and explicit models.
