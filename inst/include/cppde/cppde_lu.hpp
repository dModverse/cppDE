/*
 cppDE LU: unified Dense/Sparse iteration matrix solver.
...
private:
  ...
  void factorize_W_sparse(size_t n, value_type inv_gamma_dt)
  {
    ...
    // Structural setup, redone whenever the pattern is not the one already
    // held.  Comparing nnz alone would keep stale diagonal offsets for a
    // different pattern that happens to have the same entry count.
    if (m_W_work.n != m_W_sparse.n || m_W_work.nnz != nnz ||
      ...
      for (size_t i = 0; i < n; ++i) {
        ...
        if (pos == begin + count || *pos != static_cast<int>(i))
          throw std::runtime_error(
            "cppDE sparse LU: Jacobian pattern has no diagonal entry in "
            "column " + std::to_string(i) + "; W = 1/(gamma*h) I - J cannot "
            "be assembled.");
