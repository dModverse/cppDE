/*
 Expression Templates for cppde::dual<T, N>  (any N, T non-AD).
...
#ifndef CPPDE_DUAL_EXPR_HPP
#define CPPDE_DUAL_EXPR_HPP
...
struct DualLeaf : Expr<DualLeaf<T, N>> {
  ...
  CPPDE_ET_INLINE explicit DualLeaf(const dual<T, N>& d)
    : val_(d.x()),
      tan_(d.depend() ? &d[0] : nullptr)
  {}

  CPPDE_ET_INLINE T val() const { return val_; }
  static constexpr unsigned tan_size() { return N; }
  CPPDE_ET_INLINE bool depends() const { return tan_ != nullptr; }
  CPPDE_ET_INLINE T tan(unsigned i) const { return tan_ ? tan_[i] : T(); }
};
...
struct DualLeaf<T, 0> : Expr<DualLeaf<T, 0>> {
  ...
  CPPDE_ET_INLINE explicit DualLeaf(const dual<T, 0>& d)
    : val_(d.x()),
  ...
  CPPDE_ET_INLINE T val() const { return val_; }
  CPPDE_ET_INLINE unsigned tan_size() const { return sz_; }
  CPPDE_ET_INLINE bool depends() const { return sz_ > 0; }
  CPPDE_ET_INLINE T tan(unsigned i) const { return tan_ ? tan_[i] : T(); }
};
...
struct ScalarLeaf : Expr<ScalarLeaf<T>> {
  ...
  CPPDE_ET_INLINE explicit ScalarLeaf(T s) : s_(s) {}

  CPPDE_ET_INLINE T val() const { return s_; }
  static constexpr unsigned tan_size() { return 0u; }
  static constexpr bool depends() { return false; }
  CPPDE_ET_INLINE T tan(unsigned) const { return T(0); }
};

// =============================================================================
// Leaves and composite children are stored by value: DualLeaf is a snapshot of
// .x() and the tan_ pointer (see the note above DualLeaf).
// =============================================================================
...
struct BinExpr : Expr<BinExpr<L, R, Op>> {
  ...
  CPPDE_ET_INLINE BinExpr(L l, R r)
  ...
  CPPDE_ET_INLINE value_type val()      const { return y_; }
  CPPDE_ET_INLINE unsigned   tan_size() const { return sz_; }
  CPPDE_ET_INLINE bool       depends()  const { return dep_; }
...
struct UnaryExpr : Expr<UnaryExpr<X, Op>> {
  ...
  CPPDE_ET_INLINE explicit UnaryExpr(X x) : x_(std::move(x)) {
    Op::compute(x_.val(), y_, fp_);
  }

  CPPDE_ET_INLINE value_type val()      const { return y_; }
  CPPDE_ET_INLINE unsigned   tan_size() const { return x_.tan_size(); }
  CPPDE_ET_INLINE bool       depends()  const { return x_.depends(); }
  CPPDE_ET_INLINE value_type tan(unsigned i) const { return fp_ * x_.tan(i); }
};
...
#define CPPDE_DEFINE_ET_UNARY_OP(NAME, VAL_EXPR, FP_EXPR)                    \
  struct NAME ## Op {                                                         \
    template<class T> static void compute(const T& xv, T& y, T& fp) {         \
...
CPPDE_DEFINE_ET_UNARY_OP(Exp,   exp(xv),    y);
CPPDE_DEFINE_ET_UNARY_OP(Log,   log(xv),    T(1) / xv);
CPPDE_DEFINE_ET_UNARY_OP(Sqrt,  sqrt(xv),   T(1) / (T(2) * y));
CPPDE_DEFINE_ET_UNARY_OP(Sin,   sin(xv),    cos(xv));
CPPDE_DEFINE_ET_UNARY_OP(Cos,   cos(xv),   -sin(xv));
CPPDE_DEFINE_ET_UNARY_OP(Tan,   tan(xv),    T(1) + y * y);
CPPDE_DEFINE_ET_UNARY_OP(Asin,  asin(xv),   T(1) / sqrt(T(1) - xv * xv));
CPPDE_DEFINE_ET_UNARY_OP(Acos,  acos(xv),  -T(1) / sqrt(T(1) - xv * xv));
CPPDE_DEFINE_ET_UNARY_OP(Atan,  atan(xv),   T(1) / (T(1) + xv * xv));
CPPDE_DEFINE_ET_UNARY_OP(Sinh,  sinh(xv),   cosh(xv));
CPPDE_DEFINE_ET_UNARY_OP(Cosh,  cosh(xv),   sinh(xv));
CPPDE_DEFINE_ET_UNARY_OP(Tanh,  tanh(xv),   T(1) - y * y);
...
#undef CPPDE_DEFINE_ET_UNARY_OP
...
// scalar's leaf type matches the dual's value_type when mixing).
//
// An already-Expr operand is returned by reference; it is owned by the caller
// and outlives the node's construction.
template<class TT, class X,
         std::enable_if_t<is_dual_expr_v<X>, int> = 0>
CPPDE_ET_INLINE const X& make_leaf(const X& x, TT* /*target_tag*/ = nullptr) {
  return x;
}

template<class TT, class T, unsigned N>
CPPDE_ET_INLINE DualLeaf<T, N> make_leaf(const dual<T, N>& d, TT* /*tag*/ = nullptr) {
  return DualLeaf<T, N>(d);
}

template<class TT, class S,
         std::enable_if_t<std::is_arithmetic<S>::value, int> = 0>
CPPDE_ET_INLINE ScalarLeaf<TT> make_leaf(S s, TT* /*tag*/ = nullptr) {
  return ScalarLeaf<TT>(static_cast<TT>(s));
}

// Node type produced for operand X at target scalar TT: make_leaf's result
// decayed, for the BinExpr / UnaryExpr / PowExpr template argument.
template<class TT, class X>
using nested_node_t =
  std::decay_t<decltype(make_leaf<TT>(std::declval<const X&>()))>;
...
// =============================================================================
// make_leaf's result goes straight into the node constructor.
#define CPPDE_DEFINE_ET_BINOP(SYM, OPTAG)                                     \
  template<class A, class B,                                                   \
           std::enable_if_t<et_pair_enabled<A, B>::value, int> = 0>            \
  CPPDE_ET_INLINE auto operator SYM(const A& a, const B& b) {                 \
    using TT = typename et_pair_target<A, B>::type;                            \
    return BinExpr<nested_node_t<TT, A>, nested_node_t<TT, B>, OPTAG>(         \
             make_leaf<TT>(a), make_leaf<TT>(b));                              \
  }

CPPDE_DEFINE_ET_BINOP(+, AddOp)
CPPDE_DEFINE_ET_BINOP(-, SubOp)
CPPDE_DEFINE_ET_BINOP(*, MulOp)
CPPDE_DEFINE_ET_BINOP(/, DivOp)

#undef CPPDE_DEFINE_ET_BINOP
...
CPPDE_ET_INLINE auto operator+(const A& a) {
  using TT = expr_value_type_t<A>;
  // Identity.  Must return by value: `auto` decays make_leaf's const A&,
  // decltype(auto) would not.
  return make_leaf<TT>(a);
}
...
CPPDE_ET_INLINE auto operator-(const A& a) {
  using TT = expr_value_type_t<A>;
  return UnaryExpr<nested_node_t<TT, A>, NegOp>(make_leaf<TT>(a));
}
...
#define CPPDE_DEFINE_ET_MATH_FN(NAME, OPTAG)                                 \
  template<class A,                                                           \
           std::enable_if_t<is_et_operand<A>::value, int> = 0>                \
  CPPDE_ET_INLINE auto NAME(const A& a) {                                    \
    using TT = expr_value_type_t<A>;                                          \
    return UnaryExpr<nested_node_t<TT, A>, OPTAG>(make_leaf<TT>(a));          \
  }

CPPDE_DEFINE_ET_MATH_FN(exp,   ExpOp)
CPPDE_DEFINE_ET_MATH_FN(log,   LogOp)
CPPDE_DEFINE_ET_MATH_FN(sqrt,  SqrtOp)
CPPDE_DEFINE_ET_MATH_FN(sin,   SinOp)
CPPDE_DEFINE_ET_MATH_FN(cos,   CosOp)
CPPDE_DEFINE_ET_MATH_FN(tan,   TanOp)
CPPDE_DEFINE_ET_MATH_FN(asin,  AsinOp)
CPPDE_DEFINE_ET_MATH_FN(acos,  AcosOp)
CPPDE_DEFINE_ET_MATH_FN(atan,  AtanOp)
CPPDE_DEFINE_ET_MATH_FN(sinh,  SinhOp)
CPPDE_DEFINE_ET_MATH_FN(cosh,  CoshOp)
CPPDE_DEFINE_ET_MATH_FN(tanh,  TanhOp)
...
CPPDE_ET_INLINE auto pow(const A& a, const B& b) {
  using TT = typename et_pair_target<A, B>::type;
  return PowExprDD<nested_node_t<TT, A>, nested_node_t<TT, B>>(
           make_leaf<TT>(a), make_leaf<TT>(b));
}
...
CPPDE_ET_INLINE auto pow(const A& a, const S& s) {
  using TT = expr_value_type_t<A>;
  return PowExprDS<nested_node_t<TT, A>>(make_leaf<TT>(a), static_cast<TT>(s));
}
...
CPPDE_ET_INLINE auto pow(const S& s, const B& b) {
...
// =============================================================================
// Hoist the ET operators into namespace cppde so codegen-emitted
