...
static constexpr int    MSBJ    = 51;       // max steps between Jacobian evaluations
...
template<
  ...
  class Value = double,
  class JacobianPattern = cppde::dense_lu_tag,
...
// Exhausting the retry loop reads as failure.
m_newton_converged = false;
...
for (int nls_attempt = 0; nls_attempt < 2; ++nls_attempt) {
  ...
  if (callSetup) {
    ...
    // refactorize W) on first step, invalid J, a Newton failure
    // classified as stale-J, or once the Jacobian has aged past MSBJ
    // steps.
    bool need_new_jacobian =
      (nls_convfail == convfail_t::fail_bad_j)
    || !m_lu.has_valid_jacobian()
    || (m_nst == 0)
    || (m_nst >= m_nstlj + ndf_constants::MSBJ);
  ...
  void complete_step()
  {
    ...
    // unified n*M × (q+1) tangent matrix in m_zn_block).
    {
      ...
      if constexpr (detail::is_dual2nd<value_type>::value) {
        // dual2nd: vec_axpy_with_slab covers scalar and first-order
        // layers per element, so no separate value-layer loop runs here.
        // The Hessian layer is lifted out of the per-slot calls into one
        // dger, as the first-order tangent block already is.
        const bool block_hess =
          m_zn_block.primed() && m_zn_block.hess_slot_stride() > 0
          && m_acor_slab.hess_size() == m_zn_block.hess_slot_stride();

        for (int j = 0; j < qp1; ++j)
          vec_axpy_with_slab(m_zn[j].m_v, m_zn_block.slab(j),
                             static_cast<double>(m_l[j]),
                             m_acor.m_v, m_acor_slab,
                             /*with_hess=*/!block_hess);

        if (block_hess) {
          std::array<double, max_order + 2> ll{};
          for (int j = 0; j < qp1; ++j) ll[j] = static_cast<double>(m_l[j]);
          int M_ = static_cast<int>(m_zn_block.hess_slot_stride());
          int N_ = qp1;
          int inc = 1;
          double one = 1.0;
          F77_CALL(dger)(&M_, &N_, &one,
                         m_acor_slab.hess_data(), &inc,
                         ll.data(), &inc,
                         m_zn_block.hess_block_data(), &M_);
        }
      } else if constexpr (detail::is_dynamic_dual<value_type>::value) {
        using inner_t = typename value_type::value_type;

        // Value layer: tiled over states, slots innermost.
        {
          constexpr size_t tile = 32;
          inner_t ll[max_order + 2];
          value_type* zp[max_order + 2];
          for (int j = 0; j < qp1; ++j) {
            ll[j] = static_cast<inner_t>(m_l[j]);
            zp[j] = m_zn[j].m_v.data();
          }
          const value_type* av = m_acor.m_v.data();
          for (size_t i0 = 0; i0 < n; i0 += tile) {
            const size_t len = (n - i0 < tile) ? (n - i0) : tile;
            const value_type* a = av + i0;
            for (int j = 0; j < qp1; ++j) {
              value_type* zj = zp[j] + i0;
              const inner_t lj = ll[j];
              for (size_t t = 0; t < len; ++t) zj[t].x() += lj * a[t].x();
            }
          }
        }
        ...
        // zn[j][i] += l[j]*acor[i], inline below one tile and through
        // the per-slot daxpy above it, as in the Pascal shift.
        constexpr size_t fuse_max = 32;
        if (n <= fuse_max) {
          for (int j = 0; j < qp1; ++j) {
            const value_type lj = static_cast<value_type>(m_l[j]);
            value_type* zj = m_zn[j].m_v.data();
            const value_type* av = m_acor.m_v.data();
            for (size_t i = 0; i < n; ++i) zj[i] += lj * av[i];
          }
        } else {
          for (int j = 0; j < qp1; ++j)
            vec_axpy_with_slab(m_zn[j].m_v, m_zn_block.slab(j),
                  ...
                  const DerivIn& f0, TimeArg dt)
  {
    ...
    // restores the rows and leaves the tangent values untouched.
    if (m_n_sens != 0) prepare_sensitivities(m_n_sens);
  }
...
private:
  ...
  void apply_pascal_shift_(size_t n, bool restore)
  {
    ...
    if constexpr (detail::is_dual2nd<value_type>::value) {
      ...
      // The Hessian layer is one contiguous [hess_slot_stride x (q+1)]
      // block, shifted by a single dtrmm against the Pascal matrix, as
      // the first-order tangent block already is.
      const bool block_hess =
        m_zn_block.primed() && m_zn_block.hess_slot_stride() > 0;
      ...
      if (block_hess) {
        ...
        F77_CALL(dtrmm)(&side, &uplo, &transa, &diag,
                        ...
                        m_zn_block.hess_block_data(), &ldb
      ...
      // Value layer: tiled over states, the q(q+1)/2 slot pairs innermost.
      {
        constexpr size_t tile = 32;
        const int q = m_q;
        value_type* zp[max_order + 2];
        for (int j = 0; j <= q; ++j) zp[j] = m_zn[j].m_v.data();

        for (size_t i0 = 0; i0 < n; i0 += tile) {
          const size_t len = (n - i0 < tile) ? (n - i0) : tile;
          for (int k = 1; k <= q; ++k)
            for (int j = q; j >= k; --j) {
              value_type* lo = zp[j - 1] + i0;
              const value_type* up = zp[j] + i0;
              if (restore) { for (size_t t = 0; t < len; ++t) lo[t].x() -= up[t].x(); }
              else         { for (size_t t = 0; t < len; ++t) lo[t].x() += up[t].x(); }
            }
        }
      }
      ...
      // Inline below one tile, per-slot daxpy above it.
      constexpr size_t fuse_max = 32;
      const int q = m_q;
      if (n <= fuse_max) {
        value_type* zp[max_order + 2];
        ...
        for (int k = 1; k <= q; ++k)
          for (int j = q; j >= k; --j) {
            value_type* lo = zp[j - 1];
            const value_type* up = zp[j];
            if (restore) { for (size_t t = 0; t < n; ++t) lo[t] -= up[t]; }
            else         { for (size_t t = 0; t < n; ++t) lo[t] += up[t]; }
          }
      } else {
        const double sign = restore ? -1.0 : 1.0;
        for (int k = 1; k <= q; ++k)
...
public:
  ...
  int m_nstlp;
  int m_nstlj = 0;   // m_nst at the last Jacobian evaluation (CVODE's nstlj)
...
template<class Value, class JacobianPattern = cppde::dense_lu_tag, class Resizer = initially_resizer>
