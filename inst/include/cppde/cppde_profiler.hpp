/*
 cppDE Profiler: compile-time switchable performance instrumentation
 =====================================================================

 Zero-cost when disabled: the compiler eliminates all profiler code
 entirely when CPPDE_PROFILE is not defined.

 Two levels:
   CPPDE_PROFILE         : cumulative per-category timers (summary)
   CPPDE_PROFILE_VERBOSE : additionally prints per-step detail to stderr
 ...
 Usage in stepper/controller code:
   ...
   // Instrument a block:
   {
     auto _t = m_prof.timer(cppde::prof_cat::f_eval);
 ...
 When CPPDE_PROFILE is not defined, profiler is an empty struct,
 timer() returns a no-op, and everything compiles away.

 Copyright (C) 2026 Simon Beyer
 */

#ifndef CPPDE_PROFILER_HPP
#define CPPDE_PROFILER_HPP
...
// ============================================================================
//  Profiler implementation (active when CPPDE_PROFILE is defined)
// ============================================================================
...
public:
  ...
  void report(const char* label = "cppDE") const {
    double total = 0.0;
...
#else // CPPDE_PROFILE not defined: zero-cost stubs
