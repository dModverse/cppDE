/*
 ...
 provides interpolation at arbitrary times within the last integration
 step.

 The interpolation polynomial has degree k (current method order) and is
 constructed directly from the Nordsieck history array snapshot taken
 by the stepper right after complete_step.  Evaluation uses Horner's
...
class multistepper_dense_output
{
public:
  ...
  explicit multistepper_dense_output(
  ...
  multistepper_dense_output(const multistepper_dense_output&) = delete;
  multistepper_dense_output& operator=(const multistepper_dense_output&) = delete;

  // Movable
  multistepper_dense_output(multistepper_dense_output&&) = default;
  multistepper_dense_output& operator=(multistepper_dense_output&&) = default;
  // ====================================================================
  //  initialize
  //
  ...
  //  - NDF stepper history discarded, restart at order 1
  // ====================================================================

  void set_dense_demand(time_type next_eval, bool always, bool forward)
  {
    m_stepper.set_dense_demand(next_eval, always, forward);
  }
