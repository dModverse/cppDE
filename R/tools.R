...
## A fresh model name, unique in the session. tempfile() does not draw from the
## random stream, so set.seed() cannot repeat it.
randomModelname <- function(prefix) basename(tempfile(prefix))
...
## Drop-in for .Call() / .C() on the named DLL only. Called through do.call so
## R CMD check does not look for registrations of run-time generated entries.
.callSym <- function(sym, ...)
  do.call(.Call, c(list(sym$name), list(...), list(PACKAGE = sym$dll)))
.cSym <- function(sym, ...)
...
#'
#' @description
#' Compiles one or more C++ source files generated by [cppODE()],
#' [cvode()] or [cppFUN()] into shared libraries (`.so` or `.dll`) using
...
compile <- function(..., output = NULL, args = NULL, cores = 1, verbose = FALSE) {
  ...
  ## OpenMP, when configure could link it. Absent, the generated batch entry
  ## compiles to a serial loop via #ifdef _OPENMP.
  ...
  if (any(grepl("\\.cpp$", files)))
    message(sprintf("using C++ compiler: %s [%s]",
...
#' Match a `derivMode` argument against what an entry point supports.
#'
#' `derivMode` names which derivative products to build, and more than one may
#' be asked for: `"forward"` and `"reverse"` are separate build products and can
#' be combined.
#'
#' @param x What the caller passed.
#' @param choices The values this entry point supports.
#' @return A deduplicated subset of `choices`, in the order of `choices`.
#' @noRd
...
#' Can Code Be Generated and Compiled Here?
#'
#' @description
#' Reports whether [cppODE()], [cvode()] and [cppFUN()] can generate and
#' compile a model without installing anything first. Code generation needs
...
codegenAvailable <- function(backend = c("native", "cvode")) {
  ...
  imports <- function(py) nzchar(py) && nzchar(Sys.which(py)) &&
    identical(suppressWarnings(system2(py, c("-c", shQuote("import sympy")),
                                       stdout = FALSE, stderr = FALSE)), 0L)
  if (imports(Sys.getenv("RETICULATE_PYTHON"))) return(TRUE)
  if (dir.exists(file.path(tools::R_user_dir("reticulate", "cache"), "uv")))
    return(TRUE)
  imports("python3") || imports("python")
}
