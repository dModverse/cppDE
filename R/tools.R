...
##
## Through do.call, because R CMD check reads the first argument of a literal
## .Call() and reports a registration problem when it cannot evaluate one. These
## entry points live in shared objects generated at run time, so there is
## nothing to register and nothing to evaluate at check time.
...
#' [cppFUN()] into shared libraries (`.so` or `.dll`) using
...
#'
#' @param ... One or more objects returned by [cppODE()] or [cppFUN()]
...
#' be combined. `"symbolic"` is not a direction but an alternative backend for
#' the forward Jacobian, so it stands alone.
#'
