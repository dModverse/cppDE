// The trajectory backwards through a forcing.
//
// A forcing is the one input the generator never differentiates symbolically:
// PchipForcing interpolates data, so the chain rule through it is written by
// hand in two places, the Jacobian's dfdt and the root gradients. Nothing in
// C++ covered it in either direction, which left the reverse pass free to get
// it wrong wherever a forcing appears multiplicatively, the shape in which it
// enters df/dx rather than only dfdt.
//
...
//
// Covered: bdf, adams, rb4 and tsit5.
//
// Every number is printed at %.17g, so the output is the assertion as well.
//
// Build and run:  dev/cxx/run.sh --reverse-forcing
//
...
// The coefficients are doubles whatever V is, so one static per type holds no
// tape slot and survives every rewind.
template<class V>
static const cppde::PchipForcing<V>& forcing() {
  static const cppde::PchipForcing<V> u(
      std::vector<double>{0.0, 0.5, 1.0, 1.6, 2.2},
      std::vector<double>{0.4, 1.1, 0.7, 1.5, 0.9});
  return u;
}
...
struct model {
  ...
  void operator()(const std::vector<V>& x, std::vector<V>& d, const V& t) const {
    const V u = forcing<V>()(t);
    d[0] = -p[0] * x[0] * u + p[1] * x[1] * x[2];
    d[1] =  p[0] * x[0] * u - p[1] * x[1] * x[2] - p[2] * x[1] * x[1];
    d[2] =  p[2] * x[1] * x[1] * cppde::cos(t) + u;
  }
};
...
struct jacobian {
  ...
  void operator()(const std::vector<V>& x, cppde::dense_matrix<V>& J, const V& t,
    ...
    const V u  = forcing<V>()(t);
    const V du = forcing<V>().derivative(t);
    J(0,0) =  p[0] * u;   J(0,1) = -p[1] * x[2];  J(0,2) = -p[1] * x[1];
    J(1,0) = -p[0] * u;   J(1,1) =  p[1] * x[2] + V(2.0) * p[2] * x[1];
                          J(1,2) =  p[1] * x[1];
    J(2,0) =  V(0.0);     J(2,1) = -V(2.0) * p[2] * x[1] * cppde::cos(t);
                          J(2,2) =  V(0.0);
    // The forcing's own time dependence, the term the generator writes by hand.
    dfdt[0] = -p[0] * x[0] * du;
    dfdt[1] =  p[0] * x[0] * du;
    dfdt[2] = -p[2] * x[1] * x[1] * cppde::sin(t) + du;
  }
};
  ...
  for (std::size_t k = 0; k < store.n_steps(); ++k) {
    ...
    if constexpr (multistep) {
      ...
      if (verify_carry && k + 1 < store.n_steps()) {
        // Looser than in the forcing-free harness, and the reason is the
        // reference rather than the adjoint: under dual the corrector's
        // stopping rule maximises over the sensitivity directions, so it
        // leaves the iteration at a slightly different point than the double
        // run did, and the carry inherits that. The forcing steepens the
        // right-hand side and the drift grows with it, to about 3e-9 here.
        // Every adjoint comparison below still holds at its own tolerance.
        ...
        for (int j = 0; j <= nxt.carry.q; ++j)
          for (std::size_t i = 0; i < NX; ++i)
            close(nxt.zn[static_cast<std::size_t>(j) * NX + i], st.zn(j)[i],
                  ...
                  + " slot "
                  + std::to_string(j) + "." + std::to_string(i), 1e-7);
      }
    } else {
      x = xout;
    }
