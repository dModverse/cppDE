...
// builds, the refactorisation the pivot-growth check triggers, and the
// per-step transposed solve.
//
// Same model, same tolerances and same seeds as
// test_reverse_trajectory_methods.cpp, with sparse_lu_tag in place of
// dense_lu_tag, so the two harnesses answer the same question twice and their
// numbers are comparable. tsit5 is absent because an explicit method
// factorises nothing, rb4 because its replay cannot take a sparse Jacobian.
//
// Covered: bdf and adams, each on an adaptive run with observations
// interpolated inside the steps. rb4 cannot be: see the note in main().
//
// Every number is printed at %.17g, so the output is the assertion as well.
//
// Build and run:  dev/cxx/run.sh --reverse-sparse
//
...
// Minus the Jacobian in compressed sparse column, the way a generated sparse
// model writes it: the pattern is built once and only the values move after
// that. The structural zero at (2,2) stays out of the pattern, which is what
// makes this a sparse matrix rather than a dense one in a sparse container.
template<class V>
struct jacobian {
  std::vector<V> p;
  void operator()(const std::vector<V>& x, cppde::csc_matrix<V>& W, const V& t,
    ...
    if (!W.pattern_built) {
      ...
      W.Ax.assign(W.Ai.size(), V(0));
      W.nnz = static_cast<int>(W.Ai.size());
      W.pattern_built = true;
    }
    W.Ax[0] =  p[0];                                    // (0,0)
    W.Ax[1] = -p[0];                                    // (1,0)
    W.Ax[2] = -p[1] * x[2];                             // (0,1)
    W.Ax[3] =  p[1] * x[2] + V(2.0) * p[2] * x[1];      // (1,1)
    W.Ax[4] = -V(2.0) * p[2] * x[1] * cppde::cos(t);    // (2,1)
    W.Ax[5] = -p[1] * x[1];                             // (0,2)
    W.Ax[6] =  p[1] * x[1];                             // (1,2)
    W.Ax[7] =  V(0.0);                                  // (2,2), structural
    dfdt[0] = V(0.0);
    dfdt[1] = V(0.0);
...
static void run_method(const char* name, double tol)
{
  ...
  auto sweep_with = [&](const std::vector<double>& seeds, std::vector<double>& out) {
    cppde::reverse::equation_solver<jacobian<double>, double, true> solver(jac_d);
...
int main() {
  ...
  run_method<cppde::multistepper<multistep_method::bdf, double, cppde::sparse_lu_tag>>("bdf", 1e-6);
  run_method<cppde::multistepper<multistep_method::adams, double, cppde::sparse_lu_tag>>("adams", 1e-6);
  // rb4 is absent, and not by choice: rosenbrock4::replay_step builds its own
  // dense Jacobian and forms the residual over every n squared entry
  // (cppde_rosenbrock4.hpp:430-446), so a sparse model does not compile
  // backwards. cppODE() refuses the combination until the closed-form rb4
  // adjoint replaces that replay.
